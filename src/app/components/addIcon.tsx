...
const AddIcon = ({setView} : HomeProps) => {
    ...
    return (
        <> 
            {open &&
            <div 
                className={clsx("absolute h-[2000px] w-full top-0 left-0 bg-white/40")} 
            ...
            <div className="fixed right-0 bottom-2 space-y-2 text-white text-2xl">
                ...
                <div className="flex justify-end items-center gap-4 pr-6 [&>*]:active:bg-white [&>*]:active:transition-colors [&>*]:active:duration-200">
                    ...
                    <button 
                        className={clsx("flex justify-center items-center w-14 h-14 bg-red-500 rounded-full text-3xl shadow-[1px_2px_2px_0px_rgba(0,0,0,0.2)] animate-riseup")}
                    >
                ...
                <div className={clsx("flex justify-end items-center gap-4 [&>*]:active:bg-white [&>*]:active:transition-colors [&>*]:active:duration-200 pr-[10px]")}>
                    { open &&
                    <button 
                        className={clsx("py-1 px-4 bg-red-500 rounded-full shadow-[1px_2px_2px_0px_rgba(0,0,0,0.2)] animate-entrance")}
                    ...
                    <button 
                        ...
                        onClick={() => { open ? handleAddWorkout(): setOpen(true) }}
                    >
                        +
                    </button>
                </div>
